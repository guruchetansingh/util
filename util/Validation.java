...
//             return android.util.Patterns.PHONE.matcher(number.trim()).matches();
    ...
    /**
     * This method is used to check customerID number pattern
     *
     * @param customerID should be a valid customerID
     * @return true, if pattern matches otherwise false
     */
    private static boolean isValidCustomerID(String customerID) {
        if (Util.isStringEmpty(customerID))
            return false;
        else {
            return customerID.trim().matches(customerIDPattern);
    ...
    public static boolean isValidId(Activity activity, String value, int length) {
        ...
        if (Util.isStringEmpty(value)) {
            Util.showToast(activity, activity.getResources().getString(R.string.customerID_empty));
            return false;
        }
        ...
        if (!Validation.isValidCustomerID(value)) {
